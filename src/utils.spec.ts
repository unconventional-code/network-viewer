...
describe("utils", () => {
  it("getUrlInfo", () => {
    const url =
      "https://developer.mozilla.org/static/fonts/locales/ZillaSlab-Bold.subset.e96c15f68c68.woff2";
  ...
  it("getTimings", () => {
    ...
    expect(
      utils.getTimings(networkDataMock.log.entries[3], firstEntryTime)
    ).toMatchSnapshot();
  });
  ...
  it("prepareViewerData when intercept error exists", () => {
    expect(
      utils.prepareViewerData(interceptErrorNetworkDataMock.log.entries)
    ).toMatchSnapshot();
  });
  ...
  it("calcChartAttributes", () => {
    ...
    expect(
      utils.calcChartAttributes(data, "17256.999999999985")
    ).toMatchSnapshot();
  });
  ...
  describe("getTotalTimeOfEntry", () => {
    it("should return totalTimeOfEntry", () => {
      expect(
        utils.getTotalTimeOfEntry(networkDataMock.log.entries[0])
      ).toMatchSnapshot();
    });

    it("should work without _blocked_queueing in timings", () => {
      // eslint-disable-next-line camelcase
      const { _blocked_queueing, ...timings } =
        networkDataMock.log.entries[0].timings;
  ...
  it("formatValue", () => {
    ...
    expect(utils.formatValue("status", 0, "", { error: "ERR" })).toBe(
      "(failed)"
    );
  ...
  it("getInterceptError", () => {
    expect(
      utils.getInterceptError({ response: { _error: "ERR_TIMED_OUT" } })
    ).toBe("ERR_TIMED_OUT");
